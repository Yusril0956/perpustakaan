<div>
    <div x-data="{ showModal: false }" 
         @open-book-modal.window="showModal = true" 
         @close-book-modal.window="showModal = false"
            ...
            <section class="max-w-7xl mx-auto px-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    @foreach($recommendedBooks->take(2) as $book)
                        <div class="group relative flex gap-6 bg-parchment-light/50 border border-sepia-edge/30 p-6 hover:shadow-lg transition-all rounded-sm">
                            <img src="{{ $book->cover_url }}" class="w-24 h-32 object-cover shadow-md flex-shrink-0 border-l-2 border-black/20">
                            <div class="flex flex-col justify-between flex-1">
                                <div>
                                    <div class="flex items-center gap-1 text-[10px] tracking-widest text-coffee/80 uppercase font-bold mb-1">
                                ...
                                <button wire:click="loadBookDetail({{ $book->id }})" class="flex items-center gap-1 text-xs font-bold uppercase tracking-widest text-coffee hover:text-ink transition self-start disabled:opacity-50">
                                    <span wire:loading.remove wire:target="loadBookDetail({{ $book->id }})">Lihat Selengkapnya</span>
                ...
                <div class="max-w-7xl mx-auto px-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                        @foreach($popularBooks as $book)
                            <button type="button" wire:click="loadBookDetail({{ $book->id }})" class="group cursor-pointer text-left relative">
                                <div wire:loading wire:target="loadBookDetail({{ $book->id }})" class="absolute inset-0 z-10 bg-white/50 backdrop-blur-sm flex items-center justify-center">
                                    <span class="animate-spin text-coffee text-2xl">⚙</span>
                                </div>
                                <div class="relative overflow-hidden aspect-[3/4] mb-3 border border-sepia-edge/20 shadow-sm bg-white p-1">
                                    <img src="{{ $book->cover_url }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 sepia-[0.1]">
                                </div>
            ...
            <section class="max-w-7xl mx-auto px-6 pb-20">
                <div class="mb-12">
                    ...
                    <div class="mb-8 relative">
                        <div class="absolute left-4 top-1/2 -translate-y-1/2">
                            <x-heroicon-o-magnifying-glass wire:loading.remove wire:target="search" class="w-5 h-5 text-coffee/50" />
                            <span wire:loading wire:target="search" class="animate-spin text-coffee/50 block w-5 h-5 text-center leading-none">⚙</span>
                        </div>
                        <input wire:model.live.debounce="500ms" type="text" placeholder="Cari judul atau nama penulis..."
                ...
                @if($allBooks->count() > 0)
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                        @foreach($allBooks as $book)
                            <div class="group relative bg-white/40 backdrop-blur-sm border border-sepia-edge/30 p-4 shadow-sm hover:shadow-[4px_4px_0px_#d2b48c] transition-all duration-300 rounded-sm overflow-hidden flex flex-col h-full">
                                <div class="relative aspect-[3/4] overflow-hidden shadow-md border-l-4 border-ink/20 mb-3 bg-white p-1">
                                    <img src="{{ $book->cover_url }}" alt="{{ $book->title }}" class="w-full h-full object-cover sepia-[0.2] group-hover:sepia-0 transition-all duration-700">
                                ...
                                <div class="mt-4 pt-3 border-t border-sepia-edge/30 flex items-center justify-between">
                                    <button type="button" wire:click="loadBookDetail({{ $book->id }})"
                                        ...
                                        <span wire:loading wire:target="loadBookDetail({{ $book->id }})">Memuat...</span>
                                        <x-heroicon-o-arrow-right class="w-3 h-3 transform group-hover/btn:translate-x-1 transition-transform" />
                                    </button>
                                    ...
                                    @else
                                        <span class="text-[10px] font-bold uppercase tracking-widest text-red-800/60 border-2 border-red-800/20 border-dashed px-3 py-1 cursor-not-allowed rotate-[-2deg]">
                                            Dipinjam
                                        </span>
                                    @endif
                                </div>
                ...
                @else
                    <div class="text-center py-24 border-2 border-dashed border-sepia-edge/30 bg-white/20">
                        ...
                        <button wire:click="setTab('all')" class="px-6 py-2 text-xs uppercase font-bold tracking-wider bg-coffee text-parchment-light rounded-sm hover:bg-ink transition shadow-[3px_3px_0px_#2c2420]">
        ...
        <div x-show="showModal" style="display: none;" class="fixed inset-0 z-[9999] flex justify-end">
            ...
            <div x-show="showModal" 
                ...
                <div class="p-8">
                    <div class="flex justify-end mb-6">
                        <button @click="showModal = false" class="text-coffee/50 hover:text-red-800 transition p-2 border border-transparent hover:border-red-800/20 rounded-sm">
                    ...
                    <div class="flex flex-col items-center mb-8">
                        <div class="bg-white p-2 border border-sepia-edge/40 shadow-[4px_4px_0px_#d2b48c] mb-6 rotate-1">
                            <img src="{{ $selectedBook->cover_url }}" alt="{{ $selectedBook->title }}" class="w-32 h-48 object-cover sepia-[0.3]">
                        </div>
                        <h3 class="text-3xl font-bold italic text-ink text-center leading-tight mb-2">{{ $selectedBook->title }}</h3>
                        <p class="text-coffee font-mono text-sm tracking-wider uppercase border-b border-dashed border-sepia-edge/50 pb-4 w-full text-center">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-8">
                        <div class="border border-sepia-edge/40 p-3 bg-white/30 relative">
                            <div class="absolute -top-2 left-2 bg-[#f4ecd8] px-1 text-[9px] uppercase font-bold text-coffee/60 tracking-widest">Kategori</div>
                        ...
                        <div class="border border-sepia-edge/40 p-3 bg-white/30 relative">
                            <div class="absolute -top-2 left-2 bg-[#f4ecd8] px-1 text-[9px] uppercase font-bold text-coffee/60 tracking-widest">Status</div>
                            <div class="text-sm font-bold mt-1 flex items-center gap-2 {{ $selectedBook->can_borrow > 0 ? 'text-green-800' : 'text-red-800' }}">
                    ...
                    @if($selectedBook->isbn)
                        <div class="mb-8 p-4 border-2 border-double border-sepia-edge/30 bg-ink/5">
                            <p class="text-[10px] uppercase font-bold text-coffee/60 tracking-[0.2em] mb-1">Kode Identitas (ISBN)</p>
                    ...
                    @if($selectedBook->description)
                        <div class="mb-10">
                            <h4 class="flex items-center gap-2 text-xs uppercase font-bold text-coffee tracking-widest mb-3 border-b border-sepia-edge/30 pb-2">
                    ...
                    <div class="pt-6 border-t-2 border-dashed border-sepia-edge/50 space-y-4">
                        @auth
                            @if($selectedBook->can_borrow > 0)
                                <button type="button" wire:click="requestLoan({{ $selectedBook->id }})" wire:loading.attr="disabled"
                                    class="w-full bg-coffee text-parchment-light px-4 py-3 font-bold uppercase tracking-widest text-sm border-2 border-ink shadow-[4px_4px_0px_#2c2420] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_#2c2420] transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-wait">
                                    <span wire:loading.remove wire:target="requestLoan"><x-heroicon-o-bookmark-square class="w-5 h-5" /> Ajukan Peminjaman</span>
                                    <span wire:loading wire:target="requestLoan" class="animate-spin text-xl leading-none">⚙</span>
                                </button>
                            @else
                                <div class="w-full flex justify-center items-center gap-2 bg-red-900/10 text-red-900 px-4 py-4 font-bold uppercase tracking-widest text-sm border-2 border-red-900/20 border-dashed">
                                    <x-heroicon-o-no-symbol class="w-5 h-5" /> Maaf, stok buku ini habis.
                                </div>
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="w-full flex justify-center items-center gap-2 bg-transparent text-ink px-4 py-3 font-bold uppercase tracking-widest text-sm border-2 border-ink shadow-[4px_4px_0px_#2c2420] hover:bg-ink hover:text-parchment-light transition-all">
