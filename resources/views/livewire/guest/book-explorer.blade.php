                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                    @foreach($popularBooks as $book)
                        <button wire:key="popular-{{ $book->id }}" type="button"
                            wire:click="loadBookDetail({{ $book->id }})" class="group cursor-pointer text-left relative">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
                    ...
                    <div class="flex gap-2 flex-wrap">
                        ...
                        @foreach($categories->take(3) as $category)
                            <button wire:key="category-{{ $category->id }}" wire:click="setCategory({{ $category->id }})"
                    ...
                    @foreach($allBooks as $book)
                        <div wire:key="book-{{ $book->id }}"
                            ...
                            <div class="mt-4 pt-3 border-t border-sepia-edge/30 flex items-center justify-between">
                                <x-ui.button href="{{ route('book.detail', $book->id) }}" size="sm" variant="ghost">
                                    Lihat Detail
                                </x-ui.button>
